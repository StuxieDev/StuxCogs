...
class Welcomehook(commands.Cog):
    ...
    async def welcomehookSender(self, webhook, userObj: discord.Member, greetMessage):
        # Isolate greetMessage from welcomehookSender, and have it be passed in as argument
        ...
        try:
            ...
            print("[welcomehook] Error at welcomehookSender:")
            print("[welcomehook] >", err)
            print("[welcomehook] >", webhook, userObj, greetMessage)
            return err
    ...
    async def welcomehook(self, ctx: commands.Context):
        """Welcomehook settings

        **`[p]welcomehook settings`**\u2002View current settings

        **`[p]welcomehook setgreet`**\u2002Set a Greet/welcome message
        **`[p]welcomehook setgreethook`**\u2002 Set #channel for Greet message

        **`[p]welcomehook setleave`**\u2002Set a Leave message
        **`[p]welcomehook setleavehook`**\u2002 Set #channel for Leave message

        Due to Discord limitations, you will have to create a webhook yourself in the channel you want the welcome message in. See **`[p]welcomehook setgreethook`** for more details.

        [See Documentation >](https://stuxiedev.github.io/stux-cogs/welcomehook)
        """
        if not ctx.invoked_subcommand:
            pass

    @welcomehook.command(name="settings", aliases=["list"])
    async def welcomehooksettings(self, ctx):
        """List current Welcomehook settings"""
        guildData = await self.config.guild(ctx.guild).all()

        # Greet info
        e = discord.Embed(color=(await ctx.embed_colour()), title="Welcomehook Greet Settings")
        e.add_field(name="Greet Enabled", value=guildData.get("welcomehookEnabled", None), inline=False)
        ...
        # Leave info
        e2 = discord.Embed(color=(await ctx.embed_colour()), title="Welcomehook Leave Settings")
        ...
        if guildData.get("inviteList", None) is not None:
          e3 = discord.Embed(color=(await ctx.embed_colour()), title="Welcomehook Invite Settings", description="Invite settings found, use `[p]welcomehook invite settings` to show.")
          await ctx.send(embed=e3)

    @welcomehook.command(name="setgreethook", aliases=["set", "setchannel", "setwebhook"])
    async def welcomehooksetgreethook(self, ctx, webhookUrl):
        ...
        After you create the webhook, you can customize the profile picture and name of the "bot", which will be used when welcomehook sends a message.
        """
    ...
    @welcomehook.command(name="setleavehook")
    async def welcomehooksetleavehook(self, ctx, webhookUrl):
    ...
    @welcomehook.command(name="setgreet", aliases=["setwelcome"])
    async def welcomehooksetgreet(self, ctx, *, DiscohookJSON: str):
        ...
        [Create a webhook message here >\nSee Welcomehook help documentation >](https://stuxiedev.github.io/stux-cogs/welcomehook)
    ...
    @welcomehook.command(name="setleave")
    async def welcomehooksetleave(self, ctx, *, DiscohookJSON: str):
    ...
    @welcomehook.command(name="test")
    async def welcomehooktest(self, ctx):
        """Send a test welcome message to the welcomehook"""
        try:
            # Greet Messages
            welcomehookEnabled = await self.config.guild(ctx.guild).welcomehookEnabled()
            ...
            # Confirm
            await ctx.send("Welcomehook Greet Enabled: "+str(welcomehookEnabled))
            await ctx.send("Welcomehook Leave Enabled: "+str(leaveEnabled))

            # Send Webhooks
            try:
                greetWebhook = SyncWebhook.from_url(greetWebhook)
                await self.welcomehookSender(greetWebhook, ctx.message.author, greetMessage)
            except Exception as err:
                print("[welcomehook] Error at welcomehooktest > greetWebhook > welcomehookSender:")
                print("[welcomehook] >", err)
                print("[welcomehook] >", greetWebhook, ctx.message.author, greetMessage)
                await ctx.send("Error: Welcomehook Greet message failed. Is your webhook deleted, or your message empty?")
            try:
                leaveWebhook = SyncWebhook.from_url(leaveWebhook)
                await self.welcomehookSender(leaveWebhook, ctx.message.author, leaveMessage)
            except Exception as err:
                print("[welcomehook] Error at welcomehooktest > leaveWebhook > welcomehookSender:")
                print("[welcomehook] >", err)
                print("[welcomehook] >", leaveWebhook, ctx.message.author, greetMessage)
                await ctx.send("Error: Welcomehook Leave message failed. Is your webhook deleted, or your message empty?")
        except Exception as err:
            print("[welcomehook] Error at welcomehooktest:")
            print("[welcomehook] >", err)
            print("[welcomehook] >",
                "welcomehookEnabled:", welcomehookEnabled,
    ...
    @welcomehook.command(name="toggle")
    async def welcomehooktoggle(self, ctx, GreetOrLeave: str, TrueOrFalse: bool):
        """Enable/Disable Welcomehook Greet/Leave messages

        [p]welcomehook toggle greet true -> enable Greet messages
        [p]welcomehook toggle greet false -> disable Greet messages

        [p]welcomehook toggle leave true -> enable Leave messages
        [p]welcomehook toggle leave false -> disable Leave messages
        """
        if GreetOrLeave == "greet":
            await self.config.guild(ctx.guild).welcomehookEnabled.set(TrueOrFalse)
            return await ctx.send("Welcomehook Greet Messages is now set to "+str(TrueOrFalse))
        elif GreetOrLeave == "leave":
            await self.config.guild(ctx.guild).leaveEnabled.set(TrueOrFalse)
            return await ctx.send("Welcomehook Leave Messages is now set to "+str(TrueOrFalse))
        else:
            return await ctx.send("Error: Please specify whether you want to toggle Greet or Leave messages.")

    @welcomehook.command(name="reset")
    async def welcomehooksetclear(self, ctx, TypeTrueToConfirm: bool):
        """⚠️ Reset all settings"""
        await self.config.guild(ctx.guild).clear_raw()
        await ctx.message.add_reaction("✅")

    @welcomehook.group(name="invite", aliases=["inv", "invites"])
    @checks.admin_or_permissions(manage_guild=True)
    async def whinv(self, ctx: commands.Context):
        """Send custom Welcomehook welcomes based on invite URLs (beta)
    ...
    @whinv.command(name="add")
    async def whinvadd(self, ctx):
        ...
        try:
            ...
            print("[welcomehook] Error at whinvadd > invMsgJson:")
            print("[welcomehook] >", err)
    ...
    @whinv.command(name="edit")
    async def whinvedit(self, ctx, inviteLink: str, field: str, *, updatedContentHere: str):
        ...
        try:
            ...
            print("[welcomehook] Error at whinvedit:")
            print("[welcomehook] >", err)
            await ctx.send("Error: Could not update. Did you type it in the format:\nINVITELINKCODE   FIELD   NEW_CONTENT_HERE")

    @whinv.command(name="remove")
    async def whinvremove(self, ctx, inviteLink: str):
        ...
        if invObj is None:
            return await ctx.send("Error: Could not find invite. Is it already deleted? Run `[p]welcomehook invite sync` to clean all deleted invites from Welcomehook.")
        newInviteList = inviteList.pop(str(invObj.id), None)
        await self.config.guild(ctx.guild).inviteList.set(newInviteList)
        return await ctx.message.add_reaction("✅")

    @whinv.command(name="sync")
    async def whinvsync(self, ctx):
        ...
        for ni in newInvites:
            try:
                ...
                print("[welcomehook] Error at whinvsync:")
                print("[welcomehook] >", err)
                pass
        # Replace previous config, cache old one
    ...
    @whinv.command(name="settings", aliases=["list"])
    async def whinvsettings(self, ctx):
        ...
        for io, iv in inviteList.items():
          try:
            ...
            print("[welcomehook] Error at whinvsettings > inviteList:")
            print("[welcomehook] >", err)
            e = discord.Embed(color=(await ctx.embed_colour()), title=io, description="Data error:\n"+str(inviteList[io]))
            await ctx.send(embed=e)
        return

    @whinv.command(name="test")
    async def whinvtest(self, ctx):
            ...
            for gio in guildInvites:
                try:
                    if savedInvites[str(gio.code)] and gio.uses >= savedInvites[str(gio.code)]["uses"]:
                        ...
                        await self.welcomehookSender(invHook, userObj, savedInvites[str(gio.code)]["message"])
                        # End early if webhook exists and was sent successfully
                        # return
                except Exception as err:
                    print("[welcomehook] Error at whinvtest > guildInvites:")
                    print("[welcomehook] >", err)
                    pass
    ...
    async def on_member_join(self, userObj: discord.Member) -> None:
        ...
        # Ignore if welcomehook is disabled in the user's guild
        userGuild = userObj.guild
        welcomehookEnabled = await self.config.guild(userGuild).welcomehookEnabled()
        if welcomehookEnabled == False:
            return
        ...
        if len(savedInvites) > 0:
            ...
            for gio in guildInvites:
                try:
                        ...
                        if len(irRoles) > 0:
                            for iro in irRoles:
                                try:
                                    ...
                                    await userObj.add_roles(irAdd, reason="Welcomehook invite role "+str(gio.code))
                    ...
                    print("[welcomehook] Error at on_member_join > guildInvites:")
                    print("[welcomehook] >", err)
                    pass
        ...
        await self.welcomehookSender(webhook, userObj, greetMessage)
        return
    ...
    async def on_member_remove(self, userObj: discord.Member) -> None:
        ...
        await self.welcomehookSender(webhook, userObj, leaveMessage)
        return
