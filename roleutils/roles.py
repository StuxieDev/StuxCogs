...
class Roles(MixinMeta):
    ...
    @commands.guild_only()
    @commands.group(invoke_without_command=True, name="singlerole")
    async def _role(
        ...
        if role in member.roles and await can_run_command(ctx, "modifyrole remove"):
            com = self.bot.get_command("singlerole remove")
            await ctx.invoke(
                com,
                member=member,
                role=role,
            )
        elif role not in member.roles and await can_run_command(ctx, "singlerole add"):
            com = self.bot.get_command("singlerole add")
            await ctx.invoke(
                com,
                member=member,
                role=role,
